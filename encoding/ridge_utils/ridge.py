...
def bootstrap_ridge_3_models(models, Rresp, Presp, alphas, nboots, chunklen, nchunks,
                    corrmin=0.2, joined=None, singcutoff=1e-10, normalpha=False, single_alpha=False,
                    use_corr=True, return_wt=True, logger=ridge_logger):
    """Uses ridge regression with a bootstrapped held-out set to get optimal alpha values for each response.
    [nchunks] random chunks of length [chunklen] will be taken from the training data for each regression
    run.  [nboots] total regression runs will be performed.  The best alpha value for each response will be
    averaged across the bootstraps to estimate the best alpha for that response.
    ...
    ----------
    models : list of dict
        Each dict contains the 'Rstim' and 'Pstim' arrays for a different model.
    Rresp : array_like, shape (TR, M)
        Training responses with TR time points and M different responses (voxels, neurons, what-have-you).
        Each response should be Z-scored across time.
    Presp : array_like, shape (TP, M)
    ...
    Returns
    -------
    wt : list of array_like, shape (N, M)
        If [return_wt] is True, regression weights for N features and M responses for each model. 
        If [return_wt] is False, [].
    corrs : array_like, shape (M,)
        Validation set correlations. Predicted responses for the validation set are obtained using the regression
        weights: pred = np.dot(Pstim, wt), and then the correlation between each predicted response and each 
        column in Presp is found.
    ...
    for bi in counter(range(nboots), countevery=1, total=nboots):
        ...
        Rcmats_bi = []
        for model in models:
            RRstim = model['Rstim'][notheldinds,:]
            PRstim = model['Rstim'][heldinds,:]
            RRresp = Rresp[notheldinds,:]
            PRresp = Rresp[heldinds,:]
            ...
            Rcmats_bi.append(Rcmat)
        
        Rcmats.append(Rcmats_bi)
    
    # Find best alphas
    if nboots>0:
        allRcorrs = [np.dstack([Rcmats[bi][mi] for bi in range(nboots)]) for mi in range(len(models))]
    else:
        allRcorrs = None
    ...
    if not single_alpha:
        ...
        if joined is None:
            valphas_models = []
            for model_corrs in allRcorrs:
                meanbootcorrs = model_corrs.mean(2)
                bestalphainds = np.argmax(meanbootcorrs, 0)
                valphas_models.append(alphas[bestalphainds])
        else:
            valphas_models = []
            for model_corrs in allRcorrs:
                valphas = np.zeros((nvox,))
                for jl in joined:
                    jcorrs = model_corrs[:,jl,:].mean(1).mean(1)
                    bestalpha = np.argmax(jcorrs)
                    valphas[jl] = alphas[bestalpha]
                valphas_models.append(valphas)
    else:
        logger.info("Finding single best alpha..")
        if nboots==0:
            if len(alphas)==1:
            ...
            valphas_models = []
            for model_corrs in allRcorrs:
                meanbootcorr = model_corrs.mean(2).mean(1)
                bestalphaind = np.argmax(meanbootcorr)
                bestalpha = alphas[bestalphaind]
                valphas_models.append(np.array([bestalpha]*nvox))
            logger.info("Best alpha = %0.3f"%bestalpha)

    if return_wt:
        wts = []
        preds = []
        for model, valphas in zip(models, valphas_models):
            Rstim = model['Rstim']
            Pstim = model['Pstim']
        ...
        logger.info("Selecting predictions with highest magnitude..")
        combined_pred = np.stack(preds, axis=-1)
        max_abs_indices = np.argmax(np.abs(combined_pred), axis=-1)
        best_pred = np.take_along_axis(combined_pred, np.expand_dims(max_abs_indices, axis=-1), axis=-1).squeeze(axis=-1)

        logger.info("Calculating correlations..")
        nnpred = np.nan_to_num(best_pred)
        if use_corr:
            corrs = np.nan_to_num(np.array([np.corrcoef(Presp[:,ii], nnpred[:,ii].ravel())[0,1]
                                            for ii in range(Presp.shape[1])]))
        else:
            resvar = (Presp-best_pred).var(0)
            Rsqs = 1 - (resvar / Presp.var(0))
            corrs = np.sqrt(np.abs(Rsqs)) * np.sign(Rsqs)

        return wts, corrs, valphas_models, allRcorrs, valinds
    else:
        logger.info("Calculating correlations without weights..")
        corrs_models = []
        for model, valphas in zip(models, valphas_models):
            Rstim = model['Rstim']
            Pstim = model['Pstim']
            corrs = ridge_corr_pred(Rstim, Pstim, Rresp, Presp, valphas, 
                                    normalpha=normalpha, use_corr=use_corr,
                                    logger=logger, singcutoff=singcutoff)
            corrs_models.append(corrs)

        combined_corrs = np.stack(corrs_models, axis=-1)
        max_abs_indices = np.argmax(np.abs(combined_corrs), axis=-1)
        best_corrs = np.take_along_axis(combined_corrs, np.expand_dims(max_abs_indices, axis=-1), axis=-1).squeeze(axis=-1)

        return [], best_corrs, valphas_models, allRcorrs, valinds
